...
import l2r.gameserver.model.conditions.ConditionAgathionItemId;
...
import l2r.gameserver.model.conditions.ConditionPlayerReflectionEntry;
...
import l2r.gameserver.model.conditions.op_need_agathion;
	...
	protected Condition parsePlayerCondition(Node n, Object template)
	{
		...
		for (int i = 0; i < attrs.getLength(); i++)
		{
			...
			switch (a.getNodeName().toLowerCase())
			{
				...
				case "hasagathion":
				{
					cond = joinAnd(cond, new op_need_agathion(Boolean.parseBoolean(a.getNodeValue())));
					break;
				}
				...
				{
					final String val2 = this.getValue(a.getNodeValue(), template);
					final int type = Integer.decode(this.getValue(val2.split(":")[0], template));
					final int attempts = Integer.decode(this.getValue(val2.split(":")[1], template));

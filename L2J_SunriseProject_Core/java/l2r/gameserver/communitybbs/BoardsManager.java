...
import l2r.Config;
import l2r.features.rankPvpSystem.RPSBBSManager;
import l2r.gameserver.communitybbs.Managers.AuctionBBSManager;
import l2r.gameserver.communitybbs.Managers.CeriticationsBBSManager;
...
import l2r.gameserver.communitybbs.Managers.OlympiadBBSManager;
import l2r.gameserver.communitybbs.Managers.PartyMatchingBBSManager;
import l2r.gameserver.communitybbs.Managers.PaypalBBSManager;
...
import sunrise.services.forge.Forge;
...
public class BoardsManager
{
	public void handleCommands(L2GameClient client, String command)
	{
		...
		if (command.startsWith("_bbsrps:0"))
		{
			RPSBBSManager.getInstance().cbByPass(command, activeChar);
			return;
		}
		...
		if (command.startsWith(ClassBalanceBBSManager.CLASS_BALANCE_BBS_CMD) && activeChar.isGM())
		{
			ClassBalanceBBSManager.getInstance().cbByPass(command, activeChar);
			return;
		}
		
		if (command.startsWith(SkillsBalanceBBSManager.SKILLS_BALANCE_BBS_CMD) && activeChar.isGM())
		{
		...
		if (command.startsWith(SubClassBBSManager.getInstance()._BBSCommand))
		{
			SubClassBBSManager.getInstance().cbByPass(command, activeChar);
			return;
		}
		
		if (command.startsWith(OlympiadBBSManager.getInstance().BBS_COMMAND))
		{
			OlympiadBBSManager.getInstance().cbByPass(command, activeChar);
			return;
		}
		
		if (command.startsWith(PaypalBBSManager.getInstance()._donationBBSCommand))
		{
			PaypalBBSManager.getInstance().cbByPass(command, activeChar);
			return;
		}
		...
		if (command.startsWith("_bbsforge"))
		{
			Forge.getInstance().cbByPass(command, activeChar);
			return;
		}
		
		if (command.startsWith(RankingBBSManager.getInstance().BBS_COMMAND))
		{
			RankingBBSManager.getInstance().cbByPass(command, activeChar);
			return;
		}
		
		if (command.startsWith(CeriticationsBBSManager.getInstance()._bypass))
		{
			CeriticationsBBSManager.getInstance().cbByPass(command, activeChar);
			return;
		}
		
		if (command.startsWith(PartyMatchingBBSManager.getInstance()._BBSCommand))
		{
			PartyMatchingBBSManager.getInstance().cbByPass(command, activeChar);
			return;
		}
		
		if (command.startsWith("_bbssearch"))
		{

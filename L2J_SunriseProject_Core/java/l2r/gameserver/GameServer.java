...
import l2r.features.auctionEngine.AuctionLoader;
import l2r.features.olyranking.OlyRankingLoader;
import l2r.features.rankPvpSystem.MasterioLoader;
import l2r.features.sellBuffEngine.BuffShopLoader;
import l2r.features.subclasschanger.PartyMatchingLoader;
import l2r.features.subclasschanger.SubclassChangerLoader;
...
import l2r.gameserver.features.balanceEngine.InGameBalancerLoader;
import l2r.gameserver.features.certifications.CertificationLoader;
import l2r.gameserver.features.rebirth.RebirthLoader;
...
import l2r.gameserver.model.quest.xml.QuestsParser;
...
import l2r.gameserver.taskmanager.RandomAnimationTaskManager;
import l2r.gameserver.taskmanager.RegenTaskManager;
import l2r.gameserver.taskmanager.RespawnTaskManager;
...
import gr.sr.donationSystem.PaypalLoader;
import gr.sr.dressmeEngine.DressMeLoader;
import gr.sr.dropCalculator.DropCalcLoader;
import gr.sr.dropCalculator.DropCalculatorConfigs;
import gr.sr.fakeNpcs.xml.FakeNpcsParser;
import gr.sr.features.cancelreturn.CancelReturnLoader;
...
import gr.sr.rankEngine.RankingLoader;
...
import sunrise.services.AioBuffer.AioBufferLoader;
import sunrise.services.classAppearance.ClassAppearLoader;
import sunrise.services.forge.ForgeLoader;
...
public class GameServer
{
	...
	public GameServer() throws Exception
	{
		...
		printSection("NPCs");
		FakeNpcsParser.getInstance().load();
		...
		printSection("Sunrise Paid Systems");
		AuctionLoader.getInstance().load();
		BuffShopLoader.getInstance().load();
		CertificationLoader.getInstance().load();
		CancelReturnLoader.getInstance().load();
		ForgeLoader.getInstance().load();
		PaypalLoader.getInstance().load();
		RankingLoader.load();
		AutofarmLoader.getInstance();
		DressMeLoader.load();
		SubclassChangerLoader.getInstance().load();
		PartyMatchingLoader.getInstance().load();
		OlyRankingLoader.getInstance().load();
		InGameBalancerLoader.getInstance().load();
		RebirthLoader.getInstance().load();
		MasterioLoader.getInstance().load();
		AioBufferLoader.getInstance().load();
		ClassAppearLoader.getInstance().load();
		DropCalcLoader.getInstance().load();
		...
		// Trigger threads,
		// actually does not need to trigger them,
		// just to have in mind what is running in background
		RandomAnimationTaskManager.getInstance();
		RespawnTaskManager.getInstance();

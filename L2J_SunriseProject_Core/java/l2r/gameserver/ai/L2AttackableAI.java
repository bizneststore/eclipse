...
public class L2AttackableAI extends L2CharacterAI
{
	...
	private static final double RANDOM_WALK_RATE = 5; // confirmed
	
	// Even if rate is too high do not allow spam random walks
	private final int RANDOM_WALK_INTERVAL = NpcAIConfigs.RANDOM_WALK_INTERVAL;
	private long _lastRandomWalk = 0;
	...
	{
		AttackableAITaskManager.getInstance().add(getActiveChar());
	}
	
	@Override
	public void stopAITask()
	{
		AttackableAITaskManager.getInstance().remove(getActiveChar());
		super.stopAITask();
	}
	...
	protected void thinkActive()
	{
		...
		if (!npc.getAttackByList().isEmpty() && (Rnd.nextInt(500) == 0) && (npc.getCurrentHp() == npc.getMaxHp()) && (npc.getCurrentMp() == npc.getMaxMp()))
		{
			npc.clearAggroList();
		...
		if ((leader != null) && !leader.isAlikeDead())
		{
			...
			if (Util.calculateDistance(npc, leader, true) > (offset * 2))
			{
				int x1, y1, z1;
			...
			else if (Rnd.get(1000) < ((npc.RANDOM_WALK_RATE > 0 ? npc.RANDOM_WALK_RATE : RANDOM_WALK_RATE) * 10))
			{
		...
		else if ((npc.getSpawn() != null) && !npc.isNoRndWalk() && !npc.isMoving() && (System.currentTimeMillis() > _lastRandomWalk) && (Rnd.get(1000) < ((npc.RANDOM_WALK_RATE > 0 ? npc.RANDOM_WALK_RATE : RANDOM_WALK_RATE) * 10)))
		{
			_lastRandomWalk = System.currentTimeMillis() + RANDOM_WALK_INTERVAL;
	...
	protected void thinkAttack()
	{
		...
		if (Config.ZIG_ZAG_MOVEMENT)
		{
			if (tryZigZag(npc, mostHate))
			{
				return;
			}
		}

...
{
	protected static final int MAX_ATTACK_TIMEOUT = 30 * GameTimeController.TICKS_PER_SECOND; // 30s converted to ticks
	...
	synchronized void changeIntention(CtrlIntention intention, Object arg0, Object arg1)
	{
		...
		if (intention == AI_INTENTION_IDLE /* || intention == AI_INTENTION_ACTIVE */) // active becomes idle if only a summon is present
		{
			...
			if (intention == AI_INTENTION_IDLE)
			{
				...
				AttackableAITaskManager.getInstance().remove(getActor());
		...
		AttackableAITaskManager.getInstance().add(getActor());
	}
	...
	{
		AttackableAITaskManager.getInstance().remove(_actor);
		_actor.detachAI();
		super.stopAITask();
	}

...
public class CharEffectList
{
	...
	private final List<L2Effect> _buffs = new CopyOnWriteArrayList<>();
	/** Map containing all debuffs for this effect list. */
	private final List<L2Effect> _debuffs = new CopyOnWriteArrayList<>();
	/** They bypass most of the actions, they are not included in most operations. */
	private final List<L2Effect> _passives = new CopyOnWriteArrayList<>();
	...
	{
		// If no effect is active, return empty list
		if (isEmpty())
		{
		...
		final List<L2Effect> temp = new ArrayList<>();
	...
	public int getBuffCount()
	{
		...
		for (L2Effect e : getBuffs())
		{
			if (e.getShowIcon() && !e.getSkill().isDance() && !e.getSkill().isTriggeredSkill() && !e.getSkill().is7Signs())
			{
	...
	public int getDanceCount()
	{
		...
		for (L2Effect e : getBuffs())
		{
			if (e.getSkill().isDance() && e.getInUse() && !e.isInstant())
			{
				int skillId = e.getSkill().getId();
				if (!buffIds.contains(skillId))
				{
	...
	public int getTriggeredBuffCount()
	{
		...
		for (L2Effect e : getBuffs())
		{
			if (e.getSkill().isTriggeredSkill() && e.getInUse())
			{
				int skillId = e.getSkill().getId();
				if (!buffIds.contains(skillId))
				{
	...
	public final void stopAllEffects(boolean all)
	{
		if (all)
		{
			getEffects().stream().forEach(e -> e.exit(true));
			
			getBuffs().stream().forEach(e -> stopSkillEffects(e.getSkill().getId()));
			getDebuffs().stream().forEach(e -> stopSkillEffects(e.getSkill().getId()));
		}
		else
		{
			getEffects().stream().filter(e -> e.canBeStolen()).forEach(e -> e.exit(true));
			
			getBuffs().stream().filter(e -> e.canBeStolen()).forEach(e -> stopSkillEffects(e.getSkill().getId()));
			getDebuffs().stream().filter(e -> e.canBeStolen()).forEach(e -> stopSkillEffects(e.getSkill().getId()));
		}
	}
	...
	public final void stopAllEffectsExceptThoseThatLastThroughDeath()
	{
		...
		getEffects().stream().filter(e -> !e.getSkill().isStayAfterDeath()).forEach(e -> e.exit(true));
	}
	
	/**
	 * Exits all effects not allowed inside olympiad
	 */
	public final void stopAllEffectsNotAvailableInOly()
	{
		// Get all active skills effects from this list and exit them
		getEffects().stream().filter(e -> (e.getSkill().isBlockCertificationInOly() || e.getSkill().isBlockedInOlympiad())).forEach(e -> e.exit(true));
	}
	...
	{
		getBuffs().stream().filter(e -> e.getSkill().isToggle()).forEach(e -> e.exit());
	}
	...
	{
		getBuffs().stream().filter(e -> (e.getEffectType() == type)).forEach(e -> stopSkillEffects(e.getSkill().getId()));
		getDebuffs().stream().filter(e -> (e.getEffectType() == type)).forEach(e -> stopSkillEffects(e.getSkill().getId()));
	}
	...
	{
		getBuffs().stream().filter(e -> (e.getSkill().getId() == skillId)).forEach(e -> e.exit());
		getDebuffs().stream().filter(e -> (e.getSkill().getId() == skillId)).forEach(e -> e.exit());
	}
	...
	public void stopEffectsOnAction()
	{
		...
		{
			getBuffs().stream().filter(e -> e.getSkill().isRemovedOnAnyActionExceptMove()).forEach(e -> e.exit(true));
		}
	}
	...
	public void stopEffectsOnDamage(boolean awake)
	{
		if (_hasBuffsRemovedOnDamage)
		{
			if (hasBuffs())
			{
				for (L2Effect e : getBuffs())
				{
					if (e.getSkill().isRemovedOnDamage() && (awake || (e.getSkill().getSkillType() != L2SkillType.SLEEP)))
					{
						e.exit(true);
					}
				}
	...
	protected void addEffectFromQueue(L2Effect newEffect)
	{
		...
		{
			if (((newSkill.getAbnormalType() != AbnormalType.NONE) && !newEffect.isInstant() && _blockedBuffSlots.contains(newSkill.getAbnormalType().toString().toLowerCase())) || _blockedBuffSlots.contains(newEffect.getAbnormalType()))
			{
				return;
			}
		}
		...
		if (newSkill.isDebuff() || newSkill.isOffensive())
		{
			for (L2Effect e : getDebuffs())
			{
				if ((e.getSkill().getId() == newEffect.getSkill().getId()) && (e.getEffectType() == newEffect.getEffectType()) && (e.getAbnormalLvl() == newEffect.getAbnormalLvl()) && e.getAbnormalType().equals(newEffect.getAbnormalType()))
				{
				...
				else if (!newSkill.isHealingPotionSkill() && !newEffect.isInstant())
				{
					...
					if (effectsToRemove >= 0)
					{
						if (newSkill.getSkillType() == L2SkillType.BUFF)
						{
							for (L2Effect e : getBuffs())
							{
								if (e.getSkill().isDance() || e.getSkill().isTriggeredSkill())
								{
									continue;
								}
	...
	private void updateEffectIconsTask()
	{
		...
		if (hasBuffs())
		{
			for (L2Effect e : getBuffs())
			{
				if (!e.getShowIcon() || e.isInstant() || (e.getEffectType() == L2EffectType.SIGNET_GROUND))
				{
					continue;
				}
	...
	{
		for (L2Effect effect : _buffs)
		{
			if (effect == null)
			{
				_buffs.remove(effect);
				Log.info("CharEffectList: (buffs) leak detected and optimized");
			}
		}
		return _buffs;
	}
	...
	{
		for (L2Effect effect : _debuffs)
		{
			if (effect == null)
			{
				_debuffs.remove(effect);
				Log.info("CharEffectList: (debuffs) leak detected and optimized");
			}
		}
		return _debuffs;
	}
	...
	{
		for (L2Effect effect : _passives)
		{
			if (effect == null)
			{
				_passives.remove(effect);
				Log.info("CharEffectList: (passived) leak detected and optimized");
			}
		}
		return _passives;
	}

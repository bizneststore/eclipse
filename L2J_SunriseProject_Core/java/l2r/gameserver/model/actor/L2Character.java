...
public abstract class L2Character extends L2Object implements ISkillsHolder
{
	...
	public void teleToLocation(int x, int y, int z, int heading, int instanceId, int randomOffset)
	{
		if (isPlayer())
		{
			if (AioBufferConfigs.ALLOW_AIO_USE_GK && AIOBufferManager.isAio(getActingPlayer()))
			{
				getActingPlayer().sendMessage("AIO players are not allowed to use Gatekeepers.");
				return;
			}
		}
	...
	private boolean doAttackHitByBow(Attack attack, L2Character target, int sAtk, int reuse)
	{
		...
		if (!miss1)
		{
			...
			damage1 = (int) Formulas.calcPhysBowDam(this, target, shld1, crit1, attack.hasSoulshot());
	...
	private boolean doAttackHitByCrossBow(Attack attack, L2Character target, int sAtk, int reuse)
	{
		...
		if (!miss1)
		{
			...
			// Calculate physical damages
			damage1 = (int) Formulas.calcPhysCrossbowDam(this, target, shld1, crit1, attack.hasSoulshot());
		}
	...
	private void beginCast(L2Skill skill, boolean simultaneously, L2Character target, L2Object[] targets)
	{
		...
		else
		{
			...
			_castInterruptTime = (-hitCancelTime / GameTimeController.MILLIS_IN_TICK) // 200ms converted to ticks
			+ GameTimeController.getInstance().getGameTicks() + (hitTime / GameTimeController.MILLIS_IN_TICK);
		...
		if (!skill.isToggle())
		{
			...
			if (skill.getSkillType() == L2SkillType.ENERGY_SPEND)
			{
				final L2ItemInstance item2 = this.getInventory().getPaperdollItem(15);
				if ((item2 != null) && (item2.getAgathionEnergy() < skill.getEnergyConsume()))
				{
			...
			if (skill.getSkillType() == L2SkillType.ENERGY_REPLENISH)
			{
				final L2ItemInstance item2 = this.getInventory().getPaperdollItem(15);
				if (item2 == null)
				{
				...
				if ((item2.getItem().getAgathionMaxEnergy() - item2.getAgathionEnergy()) < skill.getEnergyConsume())
				{
	...
	public final boolean getAndSetDead()
	{
		return _isDead.getAndSet(true);
	}
	...
	public void stopAllEffectsNotAvailableInOly()
	{
		_effects.stopAllEffectsNotAvailableInOly();
	}
	...
	public void notifyDamageReceived(double damage, L2Character attacker, L2Skill skill, boolean critical, boolean damageOverTime, boolean isReflect)
	{
		...
		if (EventDispatcher.getInstance().hasListener(EventType.ON_CREATURE_DAMAGE_DEALT, attacker))
		{

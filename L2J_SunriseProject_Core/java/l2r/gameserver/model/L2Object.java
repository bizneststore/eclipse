...
import l2r.gameserver.taskmanager.AttackableAITaskManager;
...
public abstract class L2Object extends ListenersContainer implements IIdentifiable, INamable, IUniqueId, IPositionable
{
	...
	public Future<?> _summonInfoTask;
	
	public L2Object(int objectId)
	{
		setInstanceType(InstanceType.L2Object);
		_objectId = objectId;
		ObjectsStorage.put(this);
		initKnownList();
	}
	...
	public void decayMe()
	{
		...
		if (isAttackable())
		{
			ObjectsStorage.getAllPlayers().stream().filter(pc -> pc.isOnline()).forEach(pc -> pc.getKnownList().removeKnownObject(this));
		}
	}
	...
	public void deleteMe()
	{
		...
		AttackableAITaskManager.getInstance().remove(this);
		RandomAnimationTaskManager.getInstance().remove(this);
		DecayTaskManager.getInstance().cancel(this);
		
		ObjectsStorage.remove(this);
	}
	
	public void refreshID()
	{
		// Log.info("deleteMe", new IllegalArgumentException());
	...
	{
		assert (_worldRegion == null) && (getLocation2().getX() != 0) && (getLocation2().getY() != 0) && (getLocation2().getZ() != 0);
		
		synchronized (this)
		{
			// Set the x,y,z position of the L2Object spawn and update its _worldregion
			_isVisible = true;
			setWorldRegion(L2World.getInstance().getRegion(getLocation2()));
	...
	public final void spawnMe(int x, int y, int z)
	{
		...
		synchronized (this)
		{
			...
			setXYZ(x, y, z);
			setWorldRegion(L2World.getInstance().getRegion(x, y));
	...
	public void updateWorldRegion()
	{
		...
		L2WorldRegion newRegion = L2World.getInstance().getRegion(getX(), getY());
	...
	{
		final Location loc = _location.clone();// new Location(getX(), getY(), getZ(), getHeading(), getInstanceId());
		return loc;
	}
	
	/**
	 * Remember that get location2 does not return a copy of current location
	 * @return
	 */
	...
	public final void setXYZ(int newX, int newY, int newZ)
	{
		...
		try
		{
			if (L2World.getInstance().getRegion(newX, newY) != _worldRegion)
			{
				updateWorldRegion();
			}
		}
	...
	{
		return Math.sqrt(Math.pow(x - _location.getX(), 2) + Math.pow(y - _location.getY(), 2) + Math.pow(z - _z.get(), 2));
	}
	...
	public void broadcastInfo(boolean force)
	{
		...
		if (force)
		{
			for (L2Object obj : getKnownList().getKnownObjects().values())
			{
				if (obj.isPlayer() && isVisibleFor(obj.getActingPlayer()))
				{
		...
		{
			// TODO maybe we should use getKnownPlayers()....
			for (L2Object obj : getKnownList().getKnownObjects().values())
			{
				if (obj.isPlayer() && isVisibleFor(obj.getActingPlayer()))
				{

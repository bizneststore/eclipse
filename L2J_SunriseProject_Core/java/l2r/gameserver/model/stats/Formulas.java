...
public final class Formulas
{
	...
	public static double calcBlowDamage(L2Character attacker, L2Character target, L2Skill skill, byte shld, boolean ss)
	{
		...
		double ssboost = ss ? FormulasConfigs.ALT_BLOW_SS_MULTIPLIER : 1;
		double pvpBonus = 1;
		double pveBonus = 1;
		...
		double generalTraitMod = calcGeneralTraitBonus(attacker, target, skill, false);
		...
		double damage = (baseMod * criticalMod * criticalModPos * criticalVulnMod * proximityBonus * pvpBonus * pveBonus) + criticalAddMod + criticalAddVuln;
	...
	{
		return calcBlowDamage(attacker, target, skill, shld, ss);
	}
	...
	public static final double calcPhysDam(L2Character attacker, L2Character target, byte shld, boolean crit, boolean ss)
	{
		...
		double ssboost = ss ? FormulasConfigs.ALT_PHYSICAL_SKILL_SS_MULTIPLIER : 1;
		double randomMod = attacker.getRandomDamageMultiplier();
		double rangeMod = 1.00d;// getRangeMultiplier(attacker, target);
		// Trait, elements
		double weaponTraitMod = 1.00; // we dont calculate weapon traits
		...
		if (crit)
		{
			...
			criticalAddMod = ((attacker.getStat().calcStat(Stats.CRITICAL_DAMAGE_ADD, 0) * FormulasConfigs.ALT_CRITICAL_DAMAGE_ADD_MULTIPLIER * FormulasConfigs.ALT_BOOST_MELEE_MULTIPLIER) / defence);
			criticalAddVuln = ((target.calcStat(Stats.DEFENCE_CRITICAL_DAMAGE_ADD, 0, target, null) * FormulasConfigs.ALT_CRITICAL_DAMAGE_ADD_MULTIPLIER * FormulasConfigs.ALT_BOOST_MELEE_MULTIPLIER) / defence);
		...
		damage *= FormulasConfigs.ALT_BOOST_MELEE_MULTIPLIER / defence;
		...
		{
			final StatsSet set = new StatsSet();
			set.set("Formula", "Phys attack damage");
			set.set("ssboost", ssboost);
			set.set("proximityBonus", proximityBonus);
			set.set("pvpBonus", attacker.calcStat(Stats.PVP_PHYSICAL_DMG, 1, null, null));
			set.set("baseMod", baseMod);
	 ...
	 * @return
	 */
	public static final double calcPhysBowDam(L2Character attacker, L2Character target, byte shld, boolean crit, boolean ss)
	{
		double defence = target.getPDef(attacker);
		...
		double ssboost = ss ? FormulasConfigs.ALT_PHYSICAL_SKILL_SS_MULTIPLIER : 1;
		double randomMod = attacker.getRandomDamageMultiplier();
		double rangeMod = getRangeMultiplier(attacker, target);
		// Trait, elements
		double weaponTraitMod = 1.00; // we dont calculate weapon traits
		double attackTraitMod = calcAttackTraitBonus(attacker, target);
		double attributeMod = calcAttributeBonus(attacker, target, null);
		...
		{
			final StatsSet set = new StatsSet();
			set.set("Formula", "Phys bow damage");
			set.set("ssboost", ssboost);
			set.set("proximityBonus", proximityBonus);
			set.set("pvpBonus", attacker.calcStat(Stats.PVP_PHYSICAL_DMG, 1, null, null));
			set.set("baseMod", baseMod);
	 ...
	 * @return
	 */
	public static final double calcPhysCrossbowDam(L2Character attacker, L2Character target, byte shld, boolean crit, boolean ss)
	{
		double defence = target.getPDef(attacker);
	 ...
	 * @return
	 */
	public static double calcSkillPhysDam(L2Character attacker, L2Character target, L2Skill skill, byte shld, boolean crit, boolean ss)
	{
		double defence = target.getPDef(attacker);
		...
		if ((attacker instanceof L2PcInstance) || (attacker instanceof L2Summon))
		{
			...
			if ((vsAll != null) && (BalancerConfigs.CLASS_BALANCER_AFFECTS_MONSTERS || (target instanceof L2Playable)))
			{
				balancedDamage *= crit ? vsAll[6] : vsAll[5];
			}
			if ((target instanceof L2PcInstance) || (target instanceof L2Summon))
			{
				L2PcInstance t = target instanceof L2PcInstance ? target.getActingPlayer() : ((L2Summon) target).getOwner();
				...
				if (vsTarget != null)
				{
					balancedDamage *= crit ? vsTarget[6] : vsTarget[5];
				}
			}
		}
		
		double damage = baseMod * proximityBonus * pvpBonus * pveBonus;
		damage *= attackTraitMod;
		damage *= generalTraitMod;
		damage *= attributeMod;
		damage *= randomMod;
	...
	public static final double calcMagicDam(L2Character attacker, L2Character target, L2Skill skill, double mAtk, byte shld, boolean sps, boolean bss, boolean mcrit)
	{
		...
		final boolean isPvP = attacker.isPlayable() && target.isPlayable();
		final boolean isPvE = attacker.isPlayable() && target.isAttackable();
		final double shotsBonus = bss ? FormulasConfigs.ALT_MAGE_BSS_MULTIPLIER : sps ? FormulasConfigs.ALT_MAGE_SS_MULTIPLIER : 1;
		double pvpBonus = 1;
		double pveBonus = 1;
		...
		// Trait, elements
		final double generalTraitMod = calcGeneralTraitBonus(attacker, target, skill, true);
		...
		// magic_critical_dmg_diff - magical critical damage bonus. For example mag. set Аидиоса + 297 к силе маг. crit. attack Просто + к урону.
		...
		if ((attacker instanceof L2PcInstance) || (attacker instanceof L2Summon))
		{
			...
			if ((vsAll != null) && (BalancerConfigs.CLASS_BALANCER_AFFECTS_MONSTERS || (target instanceof L2Playable)))
			{
				balancedDamage *= mcrit ? vsAll[3] : vsAll[2];
			}
			if ((target instanceof L2PcInstance) || (target instanceof L2Summon))
			{
				L2PcInstance t = target instanceof L2PcInstance ? target.getActingPlayer() : ((L2Summon) target).getOwner();
				...
				if (vsTarget != null)
				{
					balancedDamage *= mcrit ? vsTarget[3] : vsTarget[2];
				}
			}
		}
		damage *= 1 + (balancedDamage / 100);
	...
	public static final double calcMagicDam(CubicInstance attacker, L2Character target, L2Skill skill, boolean mcrit, byte shld)
	{
		...
		return calcMagicDam(attacker.getOwner(), target, skill, mAtk, shld, false, false, mcrit);
	}
	...
	{
		if (target.isRaid() || target.isInvul() || target.isHpBlocked() || !target.canAbortCast())
		{
		...
		rate = calcPrayingChance(target, rate);
	...
	public static boolean calcEffectSuccess(L2Character attacker, L2Character target, EffectTemplate effect, L2Skill skill, byte shld, boolean ss, boolean sps, boolean bss)
	{
		...
		if (skill.isDebuff())
		{
			...
			if (skill.getPower() == -1)
			{
				...
				final double prayChance = calcPrayingChance(target, 0);
				if (prayChance != 0)
				{
					return (prayChance * 10) > Rnd.get(1000);
				}
				return true;
			}
		}
		
		if (target.isDebuffBlocked() && skill.hasEffectType(L2EffectType.DEBUFF, L2EffectType.STUN, L2EffectType.FEAR))
		{
			final SystemMessage sm = SystemMessage.getSystemMessage(SystemMessageId.C1_RESISTED_YOUR_S2);
			sm.addCharName(target);
			sm.addSkillName(skill);
		...
		final double traitMod = calcGeneralTraitBonus(attacker, target, skill, false);
		...
		final double rate = baseMod * elementMod * traitMod * mAtkMod * lvlBonusMod;
		...
		// vGodFather: sunrise modify success rates if target is praying
		finalRate = calcPrayingChance(target, finalRate);
		...
		try
		{
			if (attacker.isPlayer())
			{
				if (attacker.getActingPlayer().isInOlympiad() || attacker.getActingPlayer().isInOlympiadMode())
				{
					@SuppressWarnings("unused")
					String debug = "";
					debug += "Formula " + "Effect: " + skill.getName() + "( " + skill.getId() + " )";
					debug += " baseMod " + baseMod;
					debug += " elementMod " + elementMod;
					debug += " traitMod " + traitMod;
					debug += " mAtkMod " + mAtkMod;
					debug += " lvlBonusMod " + lvlBonusMod;
					debug += " rate " + String.format("%.2f", rate);
					debug += " balancedChance " + String.format("%.2f", balancedChance);
					debug += " finalRate " + String.format("%.2f", finalRate);
					debug += " result " + String.valueOf(result);
					
					// Log.LogOlySkillChances(debug);
				}
			}
		}
		catch (Exception e)
		...
		{
			final StatsSet set = new StatsSet();
			set.set("Formula", "Effect success");
			set.set("baseMod", baseMod);
	...
	public static boolean calcSkillSuccess(L2Character attacker, L2Character target, L2Skill skill, byte shld, boolean ss, boolean sps, boolean bss)
	{
		...
		double baseMod = (baseRate / statMod);
		...
		try
		{
			if (attacker.isPlayer())
			{
				if (attacker.getActingPlayer().isInOlympiad() || attacker.getActingPlayer().isInOlympiadMode())
				{
					@SuppressWarnings("unused")
					String debug = "";
					debug += "Formula " + "Skill: " + skill.getName() + "( " + skill.getId() + " )";
					debug += " baseRate " + baseRate;
					debug += " baseMod " + baseMod;
					debug += " elementMod " + elementMod;
					debug += " traitMod" + traitMod;
					debug += " mAtkMod" + mAtkMod;
	 ...
	 * @return
	 */
	private static double calcPrayingChance(L2Character target, double rate)
	{
		if ((target != null) && target.isCastingNow())
		{
			final L2Skill castingSkill = target.getLastSkillCast();
			if ((castingSkill != null) && ((castingSkill.getTargetType() == L2TargetType.FLAGPOLE) || (castingSkill.getTargetType() == L2TargetType.HOLY) || (castingSkill.getId() == 847)))
			{
				rate = 5;
			}
		}
	...
	public static boolean calcCubicSkillSuccess(CubicInstance attacker, L2Character target, L2Skill skill, byte shld)
	{
		...
		// Resist Modifier.
		double resMod = calcGeneralTraitBonus(attacker.getOwner(), target, skill, false);
		rate *= resMod;
		...
		{
			final StatsSet set = new StatsSet();
			set.set("Formula", "Cubic Skill success");
	...
	public static boolean calcMagicSuccess(L2Character attacker, L2Character target, L2Skill skill)
	{
		...
		final int gradePenalty = attacker.isPlayer() ? attacker.getActingPlayer().getExpertiseWeaponPenalty() : 0;
		...
		boolean result = (rate * 10) > Rnd.get(1000);
		
		if (attacker.isDebug())
		{
			final StatsSet set = new StatsSet();
			set.set("Formula", "Magic success");
	...
	public static double calcManaDam(L2Character attacker, L2Character target, L2Skill skill, byte shld, boolean sps, boolean bss, boolean mcrit)
	{
		...
		// Bonus Spiritshot
		mAtk *= bss ? FormulasConfigs.ALT_MAGE_BSS_MULTIPLIER : sps ? FormulasConfigs.ALT_MAGE_SS_MULTIPLIER : 1;
	...
	public static boolean calcSkillMastery(L2Character actor, L2Skill sk)
	{
		...
		if (actor.isPlayer())
		{
			...
			if (actor.isDebug())
			{
				actor.sendMessage("Skill mastery base chance: " + initVal);
			}
			...
			if (actor.isDebug())
			{
				actor.sendMessage("Skill mastery final chance: " + initVal);
			}
	...
	public static void calcDamageReflected(L2Character attacker, L2Character target, L2Skill skill, double damage, boolean crit)
	{
		...
		if (Rnd.get(100) < chance)
		{
			...
			counterdmg *= calcGeneralTraitBonus(attacker, target, skill, false);
	...
	{
		// vGodFather: sunrise always random buffs outside olympiad
		...
		switch (slot)
		{
			case "buff":
			{
				...
				/*
				 * if (!_effectList.isEmpty()) { for (L2Effect e : _effectList) { if (!e.canBeStolen() || (!calcCancelSuccess(e, cancelMagicLvl, (int) finalRate, skill))) { continue; } canceled.add(e); if (canceled.size() >= max) { break; } } }
				 */
				break;
			}
			case "debuff":
	...
	public static boolean calcProbability(double baseChance, L2Character attacker, L2Character target, L2Skill skill)
	{
		...
		return Rnd.get(1000) < ((((((skill.getMagicLevel() + baseChance) - target.getLevel()) + 30) - target.getINT()) * calcAttributeBonus(attacker, target, skill)) * calcGeneralTraitBonus(attacker, target, skill, false) * 10);
	}
	...
	public static int calcEffectAbnormalTime(L2Character caster, L2Character target, L2Effect effect)
	{
		...
		// vGodFather: implement min and max abnormal time for all 'bad' skills
		if ((caster != null) && (target != null) && (effect.getSkill().isDebuff() || effect.getSkill().isOffensive()))
		{
			...
			// vGodFather: implement min and max abnormal time
			switch (effect.getSkill().getId())
			{
				default:
					// Load default list for MaxAbnormalStateEffectTime an MinAbnormalStateEffectTime
					if (!FormulasConfigs.MIN_MAX_ABNORMAL_STATE_EFFECT_WHITE_LIST.contains(effect.getSkill().getId()))
					{
						if (caster.isPlayer() && (caster.getActingPlayer().isInOlympiad() || caster.getActingPlayer().isInOlympiadMode()))
						{
							finalTime = Math.max(Math.min(finalTime, FormulasConfigs.MAX_ABNORMAL_STATE_EFFECT_TIME_OLY), FormulasConfigs.MIN_ABNORMAL_STATE_EFFECT_TIME_OLY);
						}
						else
						{
							finalTime = Math.max(Math.min(finalTime, FormulasConfigs.MAX_ABNORMAL_STATE_EFFECT_TIME), FormulasConfigs.MIN_ABNORMAL_STATE_EFFECT_TIME);
						}
					}
					
					if (FormulasConfigs.MIN_MAX_ABNORMAL_STATE_EFFECT_BLACK_LIST.contains(effect.getSkill().getId()))
					{
						if (caster.isPlayer() && (caster.getActingPlayer().isInOlympiad() || caster.getActingPlayer().isInOlympiadMode()))
						{
							finalTime = Math.max(Math.min(finalTime, FormulasConfigs.MAX_ABNORMAL_STATE_EFFECT_TIME_BLACK_LIST_OLY), FormulasConfigs.MIN_ABNORMAL_STATE_EFFECT_TIME_BLACK_LIST_OLY);
						}
						else
						{
							finalTime = Math.max(Math.min(finalTime, FormulasConfigs.MAX_ABNORMAL_STATE_EFFECT_TIME_BLACK_LIST), FormulasConfigs.MIN_ABNORMAL_STATE_EFFECT_TIME_BLACK_LIST);
						}
					}
					
					if (FormulasConfigs.MIN_MAX_ABNORMAL_STATE_EFFECT_OTHER_LIST.contains(effect.getSkill().getId()))
					{
						if (caster.isPlayer() && (caster.getActingPlayer().isInOlympiad() || caster.getActingPlayer().isInOlympiadMode()))
						{
							finalTime = Math.max(Math.min(finalTime, FormulasConfigs.MAX_ABNORMAL_STATE_EFFECT_TIME_OTHER_LIST_OLY), FormulasConfigs.MIN_ABNORMAL_STATE_EFFECT_TIME_OTHER_LIST_OLY);
						}
						else
						{
							finalTime = Math.max(Math.min(finalTime, FormulasConfigs.MAX_ABNORMAL_STATE_EFFECT_TIME_OTHER_LIST), FormulasConfigs.MIN_ABNORMAL_STATE_EFFECT_TIME_OTHER_LIST);
						}
					}
					break;
			}
			...
			if (caster.isDebug())
			{
				...
				caster.sendMessage("originaltime: " + time);
				caster.sendMessage("finaltime: " + finalTime);
				caster.sendMessage("---------------------");
			}
		}
	...
	public static double calcAttackTraitBonus(L2Character attacker, L2Character target)
	{
		...
		if (target.isNpc())
		{
			...
			{
				case BEAST:
					weaknessBonus *= attacker.getPAtkMonsters(target);
					break;
				case ANIMAL:
					weaknessBonus *= attacker.getPAtkAnimals(target);
					break;
				case PLANT:
					weaknessBonus *= attacker.getPAtkPlants(target);
					break;
				case DRAGON:
					weaknessBonus *= attacker.getPAtkDragons(target);
					break;
				case BUG:
					weaknessBonus *= attacker.getPAtkInsects(target);
					break;
				case GIANT:
					weaknessBonus *= attacker.getPAtkGiants(target);
					break;
				case CONSTRUCT:
					weaknessBonus *= attacker.getPAtkMagicCreatures(target);
					break;
				default:
					// nothing
					break;
		...
		// System.out.println("weak bonus: " + weaknessBonus + " / " + ((L2Npc) target).getTemplate().getRace());
	...
	public static double calcGeneralTraitBonus(L2Character attacker, L2Character target, L2Skill skill, boolean ignoreResistance)
	{
		if ((skill.getTraitType() == null) || (skill.getTraitType() == L2TraitType.NONE))
		{
			return 1.0;
		}
	...
	public static int calcMAtkSpd(L2Character attacker, L2Skill skill, int skillTime)
	{
		if (skill.isMagic())
		{
			return (skillTime * 333) / Math.max(attacker.getMAtkSpd(), 1);
		}
		return (int) ((skillTime * 333) / Math.max(attacker.getPAtkSpd(), 1));
	}
	
	public static int calcSkillReuseDelay(L2Character actor, L2Skill skill)
	{
		...
		{
			return (int) (skill.getReuseDelay() * actor.calcStat(Stats.MAGIC_REUSE_RATE, 1, null, null));
		}
		else
		{
			return (int) (skill.getReuseDelay() * actor.calcStat(Stats.P_REUSE, 1, null, null));
		}
	}
	 ...
	 * @return
	 */
	private static double getRangeMultiplier(L2Character attacker, L2Character target)
	{
		final int range = attacker.getActiveWeaponItem() != null ? attacker.getActiveWeaponItem().getBaseAttackRange() : 1;
		return ((Math.min(range, attacker.calculateDistance(target, true, false)) / range) * .4) + 0.8;
	}
}

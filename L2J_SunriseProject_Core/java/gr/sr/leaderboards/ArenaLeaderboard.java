...
public class ArenaLeaderboard
{
	...
	protected int SAVETASK_DELAY = LeaderboardsConfigs.RANK_ARENA_INTERVAL;
	...
	public void formRank()
	{
		...
		Broadcast.toAllOnlinePlayers("PvP Arena Manager: " + arTop.name + " is the winner for this time with " + arTop.kills + "/" + arTop.death + ". Next calculation in " + LeaderboardsConfigs.RANK_ARENA_INTERVAL + " min(s).");
		if ((winner != null) && (LeaderboardsConfigs.RANK_ARENA_REWARD_ID > 0) && (LeaderboardsConfigs.RANK_ARENA_REWARD_COUNT > 0))
		{
			winner.getInventory().addItem("ArenaManager", LeaderboardsConfigs.RANK_ARENA_REWARD_ID, LeaderboardsConfigs.RANK_ARENA_REWARD_COUNT, winner, null);
			if (LeaderboardsConfigs.RANK_ARENA_REWARD_COUNT > 1)
			{
				winner.sendPacket(SystemMessage.getSystemMessage(SystemMessageId.EARNED_S2_S1_S).addItemName(LeaderboardsConfigs.RANK_ARENA_REWARD_ID).addInt(LeaderboardsConfigs.RANK_ARENA_REWARD_COUNT));
			}
			else
			{
				winner.sendPacket(SystemMessage.getSystemMessage(SystemMessageId.EARNED_ITEM_S1).addItemName(LeaderboardsConfigs.RANK_ARENA_REWARD_ID));
			}
	...
	public String showHtm(int owner)
	{
		...
		if ((LeaderboardsConfigs.RANK_ARENA_REWARD_ID > 0) && (LeaderboardsConfigs.RANK_ARENA_REWARD_COUNT > 0))
		{
			...
			pt += "<font color=\"aadd77\">" + LeaderboardsConfigs.RANK_ARENA_REWARD_COUNT + " &#" + LeaderboardsConfigs.RANK_ARENA_REWARD_ID + ";</font>";
		}

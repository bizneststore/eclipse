...
public class ServerVariables
{
	...
	private static void SaveToDB(String name)
	{
		try (Connection con = L2DatabaseFactory.getInstance().getConnection())
		{
			...
			if (e.isEmpty())
			{
				try (PreparedStatement statement = con.prepareStatement("DELETE FROM server_variables WHERE name = ?"))
				{
			...
			{
				try (PreparedStatement statement = con.prepareStatement("REPLACE INTO server_variables (name, value) VALUES (?,?)"))
				{
					statement.setString(1, name);

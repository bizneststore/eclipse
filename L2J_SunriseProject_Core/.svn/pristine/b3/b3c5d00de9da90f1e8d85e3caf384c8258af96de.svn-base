	...
	protected void runImpl()
	{
		...
		try
		{
			...
			if (item.isEquipable())
			{
				...
				{
					activeChar.setWeaponEquipTask(ThreadPoolManager.getInstance().scheduleGeneral(new WeaponEquipTask(item.getObjectId(), activeChar), TimeUnit.MILLISECONDS.convert(activeChar.getAttackEndTime() - System.nanoTime(), TimeUnit.NANOSECONDS)));
				}
				else
				{
					...
					{
						final L2Character target = activeChar.getAI().getInteractionTarget();

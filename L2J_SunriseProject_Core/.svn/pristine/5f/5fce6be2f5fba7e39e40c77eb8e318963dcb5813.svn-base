...
public class L2Clan extends L2Object
{
	...
	public L2Clan(int clanId)
	{
		...
		// Initialize the clan stats module
	...
	public void addClanMember(L2PcInstance player)
	{
		...
		// Update the clan stats with the new total member count
		getStats().addClanStats(Ranking.STAT_TOP_CLAN_MEMBERS_COUNT.getRankingInfo(), getMembersCount());
		
		// Add a new recruited member to the stats
	...
	public void removeClanMember(int objectId, long clanJoinExpiryTime)
	{
		...
		// We add a new member that withdrew from the clan to the stats
	...
	public void setLevel(int level)
	{
		...
		if ((_level >= 2) && (_forum == null) && (Config.ENABLE_COMMUNITY))
		{
			...
			if (forum != null)
			{
				_forum = forum.getChildByName(getName());
				if (_forum == null)
				{
					_forum = ForumsBBSManager.getInstance().createNewForum(getName(), ForumsBBSManager.getInstance().getForumByName("ClanRoot"), Forum.CLAN, Forum.CLANMEMBERONLY, getId());
				}
			}
		}
	}
	...
	public void updateClanInDB()
	{
			...
			PreparedStatement ps = con.prepareStatement("UPDATE clan_data SET leader_id=?,ally_id=?,ally_name=?,reputation_score=?,ally_penalty_expiry_time=?,ally_penalty_type=?,char_penalty_expiry_time=?,dissolving_expiry_time=?,new_leader_id=? WHERE clan_id=?"))
		{
			...
			// Save all clan stats to the database
			...
			PreparedStatement statement = con.prepareStatement(SELECT_CLAN_DATA))
		{
			...
			try (ResultSet clanData = statement.executeQuery())
			{
				if (clanData.next())
				{
					...
					try (
						PreparedStatement select = con.prepareStatement("SELECT char_name,level,classid,charId,title,power_grade,subpledge,apprentice,sponsor,sex,race FROM characters WHERE clanid=? UNION SELECT char_name,level,classid,obj_Id,title,power_grade,subpledge,apprentice,sponsor,sex,race FROM autobots WHERE clanid=?"))
					{
						select.setInt(1, getId());
						select.setInt(2, getId());
			...
			// Restore all clan stats from the database
	...
	public synchronized void addReputationScore(int value, boolean save)
	{
		...
		try
		{
			// Add the new reputation to the stats
	...
	public boolean levelUpClan(L2PcInstance player)
	{
		...
		// Notify to scripts
		if (EventDispatcher.getInstance().hasListener(EventType.ON_PLAYER_CLAN_LVLUP))
		{
	...
	public Map<Integer, L2ClanMember> getMembersMap()
	{
		return _members;
	}
	
	// Clan Stats support for Server Ranking

...
public final class L2WorldRegion
{
	...
	private final Map<Integer, L2DoorInstance> _doors = new ConcurrentHashMap<>(1);
	...
	private void switchAI(boolean isOn)
	{
		...
		if (!isOn)
		{
			for (L2Object o : _visibleObjects.values())
			{
				...
				{
					RandomAnimationTaskManager.getInstance().remove(o);
				}
			}
		...
		else
		{
			for (L2Object o : _visibleObjects.values())
			{
				if (o instanceof L2Attackable)
				{
					...
					RandomAnimationTaskManager.getInstance().add((L2Npc) o);
				}
				else if (o instanceof L2Npc)
				{
					RandomAnimationTaskManager.getInstance().add((L2Npc) o);
	...
	public void addVisibleObject(L2Object object)
	{
		...
		if (object.isDoor())
		{
			addDoor((L2DoorInstance) object);
			
			for (L2WorldRegion nRegion : getSurroundingRegions())
			{
				nRegion.addDoor((L2DoorInstance) object);
			}
		}
		else if (object instanceof L2Playable)
		{
	...
	public void removeVisibleObject(L2Object object)
	{
		...
		if (object.isDoor())
		{
			removeDoor((L2DoorInstance) object);
			
			for (L2WorldRegion nRegion : getSurroundingRegions())
			{
				nRegion.removeDoor((L2DoorInstance) object);
			}
		}
		else if (object instanceof L2Playable)
		{
	...
	{
		// Add only world doors
		if (door.getInstanceId() != 0)
		{
			return;
		}
		
		_doors.put(door.getTemplate().getId(), door);
	}
	
	private void removeDoor(L2DoorInstance door)
	{
		_doors.remove(door.getTemplate().getId());
	}

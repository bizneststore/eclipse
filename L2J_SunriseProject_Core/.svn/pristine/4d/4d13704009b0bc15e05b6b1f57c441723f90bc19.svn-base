...
public final class L2ItemInstance extends L2Object
{
	...
	public final void pickupMe(L2Character player)
	{
		...
		{
			// Notify to scripts
			if (EventDispatcher.getInstance().hasListener(EventType.ON_PLAYER_ITEM_PICKUP, getItem()))
			{
	...
	public boolean setAugmentation(L2Augmentation augmentation)
	{
		...
		// Notify Scripts
		if (EventDispatcher.getInstance().hasListener(EventType.ON_PLAYER_AUGMENT, getItem()))
		{
	...
	public Elementals getElement()
	{
		...
		{
			return getItem().getElementals()[0];
		}
		else if (_elementals != null)
		{
			return _elementals[0];
		}
		return null;
	}
	...
	public class ItemDropTask implements Runnable
	{
		...
		public final void run()
		{
			...
			if (Config.GEODATA)
			{
				...
				if ((_dropper != null) && !GeoData.getInstance().canMoveToCoord(_dropper, _x, _y, _z, true))
				{
					final Location dropDest = GeoData.getInstance().moveCheck(_dropper, new Location(_x, _y, _z), true);
					_x = dropDest.getX();
					_y = dropDest.getY();
					_z = dropDest.getZ();
				}
	...
	{
		// vGodFather: we need some extra time if item is equipped to set all values properly in world
		ThreadPoolManager.getInstance().scheduleGeneral(new ItemDropTask(this, dropper, x, y, z), 100);
		if ((dropper != null) && dropper.isPlayer())
		{
			// Notify to scripts
			if (EventDispatcher.getInstance().hasListener(EventType.ON_PLAYER_ITEM_DROP, getItem()))
			{
	...
	{
		private final L2ItemInstance _energyItem;
		
		public ScheduleConsumeEnergyTask(final L2ItemInstance item)
		{
			_energyItem = item;
		}
	...
	public void onBypassFeedback(L2PcInstance activeChar, String command)
	{
		if (command.startsWith("Quest"))
		{
			...
			if (event != null)
			{
				if (EventDispatcher.getInstance().hasListener(EventType.ON_ITEM_BYPASS_EVENT, getItem()))
				{
					EventDispatcher.getInstance().notifyEventAsync(new OnItemBypassEvent(this, activeChar, event), getItem());
				}
			}
			else if (EventDispatcher.getInstance().hasListener(EventType.ON_ITEM_TALK, getItem()))
			{
	...
	public void setAgathionEnergy(final int agathionEnergy)
	{
		this._agathionEnergy = agathionEnergy;
	}
	
	public void decreaseEnergy(final boolean resetConsumingEnergy)
	{
		this.decreaseEnergy(resetConsumingEnergy, 1);
	}
	
	public void decreaseEnergy(final boolean resetConsumingEnergy, final int count)
	{
	...
	public void scheduleConsumeEnergyTask()
	{
		...
		ThreadPoolManager.getInstance().scheduleGeneral(new ScheduleConsumeEnergyTask(this), ENERGY_CONSUMPTION_RATE);
	}
	...
	public void applyEnchantStats()
	{
		...
		// We must remove old stats before add new

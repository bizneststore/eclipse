...
public class L2DoorTemplate extends L2CharTemplate implements IIdentifiable
{
	...
	private final Line2D _line = new Line2D.Double();
	...
	public L2DoorTemplate(StatsSet set)
	{
		...
		double dx1 = _nodeX[0] - _nodeX[1];
		double dy1 = _nodeY[0] - _nodeY[1];
		double dx2 = _nodeX[1] - _nodeX[2];
		double dy2 = _nodeY[1] - _nodeY[2];
		
		if (((dx1 * dx1) + (dy1 * dy1)) > ((dx2 * dx2) + (dy2 * dy2)))
		{
			int x1 = ((_nodeX[0] + _nodeX[3]) / 2);
			int x2 = ((_nodeX[1] + _nodeX[2]) / 2);
			int y1 = ((_nodeY[0] + _nodeY[3]) / 2);
			int y2 = ((_nodeY[1] + _nodeY[2]) / 2);
			_line.setLine(x1, y1, x2, y2);
		}
		else
		{
			int x1 = ((_nodeX[0] + _nodeX[1]) / 2);
			int x2 = ((_nodeX[2] + _nodeX[3]) / 2);
			int y1 = ((_nodeY[0] + _nodeY[1]) / 2);
			int y2 = ((_nodeY[2] + _nodeY[3]) / 2);
			_line.setLine(x1, y1, x2, y2);
		}
	}

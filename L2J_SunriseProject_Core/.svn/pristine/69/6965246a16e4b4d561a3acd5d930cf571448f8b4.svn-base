...
import l2r.gameserver.features.certifications.CertEmergentStats;
...
public class CharStat
{
	...
	public int getMAtk(L2Character target, L2Skill skill)
	{
		...
		{
			attack *= Config.L2JMOD_CHAMPION_ATK;
		}
		
		if (_activeChar.isRaid())
		{
			attack *= Config.RAID_MATTACK_MULTIPLIER;
		}
		
		if (_activeChar.isPlayer())
		{
			int mAtk = (int) calcStat(Stats.MAGIC_ATTACK, attack, target, skill);
			mAtk += (mAtk / 100) * CertFunc.getCertStatMul(_activeChar.getActingPlayer(), CertEmergentStats.MAGIC_ATTACK);
			return mAtk;
		}
	...
	public int getMDef(L2Character target, L2Skill skill)
	{
		...
		if (_activeChar.isPlayer())
		{
			int mDef = (int) calcStat(Stats.MAGIC_DEFENCE, defence, target, skill);
			mDef += (mDef / 100) * CertFunc.getCertStatMul(_activeChar.getActingPlayer(), CertEmergentStats.MAGIC_DEFENCE);
			return mDef;
		}
	...
	{
		// Get the base PAtk of the L2Character
		double attack = _activeChar.getTemplate().getBasePAtk();
		...
		if (_activeChar.isRaid())
		{
			attack *= Config.RAID_PATTACK_MULTIPLIER;
		}
		
		if (_activeChar.isPlayer())
		{
			int pAtk = (int) calcStat(Stats.POWER_ATTACK, attack, target, null);
			pAtk += (pAtk / 100) * CertFunc.getCertStatMul(_activeChar.getActingPlayer(), CertEmergentStats.POWER_ATTACK);
			return pAtk;
		}
		
		return (int) calcStat(Stats.POWER_ATTACK, attack, target, null);
	}
	...
	public int getPDef(L2Character target)
	{
		...
		if (_activeChar.isPlayer())
		{
			int pDef = (int) calcStat(Stats.POWER_DEFENCE, defence, target, null);
			pDef += (pDef / 100) * CertFunc.getCertStatMul(_activeChar.getActingPlayer(), CertEmergentStats.POWER_DEFENCE);
			return pDef;
		}

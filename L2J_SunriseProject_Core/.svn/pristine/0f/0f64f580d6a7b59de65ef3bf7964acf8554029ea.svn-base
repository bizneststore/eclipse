...
public final class L2ChestInstance extends L2MonsterInstance
{
	...
	public static final SkillHolder[] TREASURE_BOMBS = new SkillHolder[]
	{
	...
	// TODO unhardcode me
	public static final Map<Integer, List<ItemChanceHolder>> DROPS = new HashMap<>();
	...
	public void tryOpen(final L2PcInstance opener, final L2Skill skill)
	{
		final double chance = this.calcChance(opener, skill);
	...
	private void tryDrop(L2PcInstance attacker)
	{
		final List<ItemChanceHolder> items = DROPS.get(getId());
		if (items == null)
		{
			_log.warn("Tresure Chest ID " + getId() + " doesn't have a drop list!");
		}
		else
		{
			...
			{
				if (Rnd.get(10000) < item.getChance())
				{
					dropItem(attacker, item.getId(), item.getCount());
				}
			}
		}
	}
	
	public double calcChance(final L2PcInstance opener, final L2Skill skill)
	{
		double chance = skill.getActivateRate();
		final int npcLvl = this.getLevel();
		final double levelmod = skill.getMagicLevel() - npcLvl;
		chance += levelmod * 1;
	...
	private void fakeOpen(final L2Character opener)
	{
		addDamageHate(this, 0, 1000000);
		
		setTarget(this);
		
		final L2Skill skill = L2ChestInstance.TREASURE_BOMBS[getLevel() / 10].getSkill();
		getAI().setIntention(CtrlIntention.AI_INTENTION_CAST, skill, this);
		
		opener.broadcastSocialAction(13);
	}
}

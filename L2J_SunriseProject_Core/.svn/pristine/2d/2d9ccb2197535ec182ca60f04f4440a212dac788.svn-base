...
public class L2PcHenna extends PcExtention
{
	...
	public boolean removeHenna(int slot)
	{
		...
		// Notify to scripts
		if (EventDispatcher.getInstance().hasListener(EventType.ON_PLAYER_HENNA_REMOVE, getChar()))
		{
	...
	public boolean addHenna(L2Henna henna)
	{
		for (int i = 0; i < 3; i++)
		{
			if (_henna[i] == null)
			{
				...
				// Notify to scripts
				if (EventDispatcher.getInstance().hasListener(EventType.ON_PLAYER_HENNA_ADD, getChar()))
				{
					EventDispatcher.getInstance().notifyEventAsync(new OnPlayerHennaAdd(getChar(), henna), getChar());
				}
				return true;
			}
		}
	...
	public boolean addDontStore(L2Henna henna)
	{
		for (int i = 0; i < 3; i++)
		{
			if (_henna[i] == null)

...
public final class QuestState
{
	...
	public QuestState exitQuest(boolean repeatable)
	{
		...
		// Add a new quest finished to the stats
	...
	public void calcExpAndSp(final int questId)
	{
		Quest.calcExpAndSp(this._player, questId);
	}
	
	public void calcReward(final int questId)
	{
		Quest.calcReward(this._player, questId);
	}
	
	public void calcReward(final int questId, final int variant)
	{
		Quest.calcReward(this._player, questId, variant);
	}
	
	public void calcRewardPerItem(final int questId, final int variant, final int totalAmount)
	{
		Quest.calcRewardPerItem(this._player, questId, variant, totalAmount);
	}
	
	public void calcRewardPerItem(final int questId, final int variant, final int totalAmount, final boolean isRandom)
	{
		Quest.calcRewardPerItem(this._player, questId, variant, totalAmount, isRandom);
	}
	
	public void calcReward(final int questId, final int variant, final boolean isRandom)
	{
		Quest.calcReward(this._player, questId, variant, isRandom);
	}
	
	public boolean calcDropItems(final int questId, final int itemId, final int npcId, final int limit)
	{
				...
				for (final QuestDropItem drop : dropList)
				{
					if ((drop != null) && (drop.getId() == itemId))
					{
						if (Rnd.chance(drop.getChance()))
						{
							...
							count *= (long) (drop.isRateable() ? Config.RATE_QUEST_DROP : drop.getRate());
							break;
						}
						break;
					}
		...
		if (count > 0L)
		{
			...
			if (count > 0L)
			{
				...
				{
					this.playSound(QuestSound.ITEMSOUND_QUEST_MIDDLE);
					return true;
				}
				this.playSound(QuestSound.ITEMSOUND_QUEST_ITEMGET);
			}
		}
		return false;
	}
	
	public void calcDoDropItems(final int questId, final int itemId, final int npcId, final int limit)
	{
		if ((limit <= 0) || (itemId <= 0) || (npcId <= 0))
		{
			return;

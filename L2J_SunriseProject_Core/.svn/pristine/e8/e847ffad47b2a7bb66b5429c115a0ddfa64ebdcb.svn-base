...
public class L2Spawn implements IPositionable, IIdentifiable, INamable
{
	...
	private final ConcurrentSkipListMap<Integer, L2Npc> _spawnedNpcs = new ConcurrentSkipListMap<>();
	...
	public void decreaseCount(L2Npc oldNpc)
	{
		...
		_spawnedNpcs.remove(oldNpc.getObjectId());
	...
	private L2Npc initializeNpcInstance(L2Npc mob)
	{
		...
		_spawnedNpcs.put(mob.getObjectId(), mob);
		if (_lastSpawnPoints != null)
		{
	...
	{
		final Entry<Integer, L2Npc> entry = _spawnedNpcs.lastEntry();
		if (entry == null)
		{
			return null;
		}
	...
	public final Collection<L2Npc> getSpawnedNpcs()
	{
		for (L2Npc npc : _spawnedNpcs.values())
		{
			if (ObjectsStorage.getObject(npc.getObjectId()) == null)
			{
				// Log.info("L2Spawn : NONE EXISTING OBJECT IN WORLD: " + npc);
				_spawnedNpcs.remove(npc.getObjectId());
			}
		}
		
		return _spawnedNpcs.values();
	}

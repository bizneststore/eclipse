...
public class Duel
{
	...
	private void sendEndMessages()
	{
		...
		switch (checkEndDuelCondition())
		{
			...
			case TEAM_2_SURRENDER:
				...
				// Add to the stats the won and lost duel for each part
				setWinDuel(1);
				setLostDuel(2);
				break;
	...
	/**
	 * Sets stats for the winner team or player for the duel
	 * @param nTeam
	 */
	public void setWinDuel(int nTeam)
	{
		if (nTeam == 1)
		{
			...
			if (_partyDuel && (_leaderA.getParty() != null))
			{
				for (L2PcInstance temp : _leaderA.getParty().getMembers())
				{
			...
			{
				_leaderA.addPlayerStats(Ranking.STAT_TOP_DUELS_WIN);
			}
		}
		else
		{
			...
			if (_partyDuel && (_leaderB.getParty() != null))
			{
				for (L2PcInstance temp : _leaderB.getParty().getMembers())
				{
					temp.addPlayerStats(Ranking.STAT_TOP_DUELS_WIN);
				}
			}
			...
			{
				_leaderB.addPlayerStats(Ranking.STAT_TOP_DUELS_WIN);
			}
		}
	}
	
	/**
	 * Sets stats for the losing team or player for the duel
	 * @param nTeam
	 */
	public void setLostDuel(int nTeam)
	{
		if (nTeam == 1)
		{
			...
			{
				_leaderA.addPlayerStats(Ranking.STAT_TOP_DUELS_LOST);
			}
		}
		else
		{
			...
			{
				_leaderB.addPlayerStats(Ranking.STAT_TOP_DUELS_LOST);
			}
		}
	}

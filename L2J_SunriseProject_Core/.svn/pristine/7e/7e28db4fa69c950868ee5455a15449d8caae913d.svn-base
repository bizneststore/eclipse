...
public class CharSelectionInfo extends L2GameServerPacket
{
	...
	protected final void writeImpl()
	{
		...
		for (int i = 0; i < size; i++)
		{
			...
			writeD(Integer.parseInt(charInfoPackage.getVar("donate_race", "-1")) >= 0 ? Integer.parseInt(charInfoPackage.getVar("donate_race", "0")) : charInfoPackage.getRace());
			writeD(Integer.parseInt(charInfoPackage.getVar("donate_class", "-1")) >= 0 ? Integer.parseInt(charInfoPackage.getVar("donate_class", "0")) : charInfoPackage.getBaseClassId());
			...
			PreparedStatement offline = con.prepareStatement("SELECT * FROM sunrise_variables WHERE obj_id = ?"))
		{
			...
			try (ResultSet rs = offline.executeQuery())
			{
				while (rs.next())
				{
					...
					charInfopackage.user_variables.put(varName, value);
				}
			}
		}
		catch (Exception e)

...
public abstract class ItemContainer
{
	...
	public L2ItemInstance[] getItems()
	{
		...
		for (L2ItemInstance item : _items)
		{
			if (item == null)
			{
				Log.error("ItemContainer: memory leak detected and optimized");
				_items.remove(item);
				continue;
			}
			
			final L2ItemInstance itemstored = ObjectsStorage.getAsItem(item.getObjectId());
			if (itemstored == null)
			{
				Log.error("ItemContainer: none existing item: " + item);
				_items.remove(item);
				continue;
			}
			list.add(item);
...
//			item.deleteMe();
	...
	public long getFAdena()
	{
		...
		for (L2ItemInstance item : getItems())
		{
			if (item.getId() == CustomServerConfigs.ALTERNATE_PAYMENT_ID)
			{
				count = item.getCount();
				return count;
			}

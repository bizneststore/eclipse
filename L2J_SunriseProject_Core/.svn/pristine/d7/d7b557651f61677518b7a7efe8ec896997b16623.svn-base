...
import l2r.Config;
import l2r.features.rankPvpSystem.RPSBypass;
	...
	protected void runImpl()
	{
		...
		try
		{
			...
			else if (_command.startsWith("manor_menu_select"))
			{
				...
				if (Config.ALLOW_MANOR && (lastNpc != null) && lastNpc.canInteract(activeChar) && EventDispatcher.getInstance().hasListener(EventType.ON_NPC_MANOR_BYPASS, lastNpc))
				{
			...
			{
				AutobotsManager.INSTANCE.onBypass(activeChar, _command);
		...
		if (EventDispatcher.getInstance().hasListener(EventType.ON_PLAYER_BYPASS, activeChar))
		{

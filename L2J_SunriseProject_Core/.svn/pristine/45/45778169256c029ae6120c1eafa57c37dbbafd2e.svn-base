...
import l2r.features.rankPvpSystem.RPSConfig;
import l2r.features.rankPvpSystem.RPSCookie;
...
import l2r.gameserver.features.certifications.CertFunc;
...
import l2r.gameserver.model.actor.instance.PcInstance.PcData;
...
import l2r.gameserver.taskmanager.PvpFlagTaskManager;
...
import sunrise.services.AioBuffer.AIOBufferManager;
import sunrise.services.AioBuffer.AioBufferConfigs;
...
public class L2PcInstance extends L2Playable
{
	...
	protected long _onlineTime;
	protected long _onlineBeginTime;
	...
	protected boolean _observerMode = false;
	...
	private final AtomicBoolean _isHourglassEffected = new AtomicBoolean(false);
	private final AtomicBoolean _isRecomTimerActive = new AtomicBoolean(false);
	/** Recommendation task **/
	...
	private Future<?> _weaponEquipTask;
	
	private void cancelWeaponEquipTask()
	{
		if (_weaponEquipTask != null)
		{
			_weaponEquipTask.cancel(true);
			_weaponEquipTask = null;
		}
	}
	
	public void setWeaponEquipTask(Future<?> task)
	{
		cancelWeaponEquipTask();
		_weaponEquipTask = task;
	}
	...
	public static L2PcInstance create(L2PcTemplate template, String accountName, String name, PcAppearance app, int classId)
	{
		// Create a new L2PcInstance with an account name
		L2PcInstance player = new L2PcInstance(IdFactory.getInstance().getNextId(), template, accountName, app);
		// Set the name of the L2PcInstance
	...
	public L2PcInstance(int objectId, L2PcTemplate template, String accountName, PcAppearance app)
	{
		...
		// Create the statics holder for this pc
	...
	public void refreshExpertisePenalty(boolean broadcast)
	{
		...
		{
			if (item.isEquipped() && ((item.getItemType() != EtcItemType.ARROW) && (item.getItemType() != EtcItemType.BOLT)))
			{
	...
	public void useEquippableItem(L2ItemInstance item, boolean abortAttack)
	{
		...
		// Notify to scripts
		if (EventDispatcher.getInstance().hasListener(EventType.ON_PLAYER_EQUIP_ITEM, this))
		{
	...
	public void setFame(int fame)
	{
		...
		// Add the fame acquired to the stats
		if (fame > _fame)
		{
	...
	{
		if (AIOBufferManager.isAio(this))
		{
			return;
		}
	...
	{
		if (!AioBufferConfigs.AIO_NO_ATTACK_IN_OTHERS_ZONE)
		{
			if (AIOBufferManager.isAio(this) && (!isInsideZone(ZoneIdType.PEACE) || !target.isInsideZone(ZoneIdType.PEACE)))
			{
			...
			if (target instanceof L2PcInstance)
			{
				if (AIOBufferManager.isAio(target.getActingPlayer()) && !isInsideZone(ZoneIdType.PEACE))
				{
					sendPacket(SystemMessage.getSystemMessage(SystemMessageId.INCORRECT_TARGET));
					sendPacket(ActionFailed.STATIC_PACKET);
					return;
	...
	public void onKillUpdatePvPKarma(L2Character target)
	{
		...
		{
			// Add the arena kill to the stats
			if (target.isPlayer())
			{
		...
		targetPlayer.isInsideZone(ZoneIdType.ZONE_CHAOTIC))) // Target player is inside chaotic zone
		{
			...
			// Add the pvp death to the stats
			if (target.isPlayer())
			{
	...
	public void increasePvpKills(L2Character target, boolean event)
	{
		if (target instanceof L2PcInstance)
		{
			if (event || (AntiFeedManager.getInstance().check(this, target) && (!RPSConfig.RANK_PVP_SYSTEM_ENABLED || (RPSConfig.RANK_PVP_SYSTEM_ENABLED && !RPSConfig.PVP_COUNTER_FOR_ALTT_ENABLED))))
			{
				...
				// Adds 1 pvp kill to the stats
				addPlayerStats(Ranking.STAT_TOP_PVP_KILLS);
				
				// If the player has a clan, then we must add a new pvp kill to the clan stats
				if (getClan() != null)
				{
	...
	public void updatePvPStatus(L2Character target)
	{
		...
		// do not flag player if target is self
	...
	protected boolean checkAndEquipArrows()
	{
		...
		else
		{
			...
			// We are going to revalidate equipped arrows
			if (_arrowItem != null)
			{
				if (_arrowItem.getItem().getItemGradeSPlus() != getActiveWeaponItem().getItemGradeSPlus())
				{
					_arrowItem = getInventory().findArrowForBow(getActiveWeaponItem());
					if (_arrowItem != null)
					{
	...
	protected boolean checkAndEquipBolts()
	{
		...
		else
		{
			...
			if (_boltItem != null)
			{
				if (_boltItem.getItem().getItemGradeSPlus() != getActiveWeaponItem().getItemGradeSPlus())
				{
					_boltItem = getInventory().findBoltForCrossBow(getActiveWeaponItem());
					if (_boltItem != null)
					{
	...
	private static L2PcInstance restore(int objectId)
	{
			...
			PreparedStatement statement = con.prepareStatement(RESTORE_CHARACTER))
		{
			...
			try (ResultSet rset = statement.executeQuery())
			{
				if (rset.next())
				{
					...
					// Get dynamically characters values getting names from the ranking
		...
		// Now we must get all the stats from the alternative table, using the ranking values
		try (Connection con = L2DatabaseFactory.getInstance().getConnection();
	...
	protected void storeCharBase()
	{
		long totalOnlineTime = _onlineTime;
		final long currenttimeonline = _onlineBeginTime == 0 ? 0 : (System.currentTimeMillis() - _onlineBeginTime) / 1000;
		totalOnlineTime += currenttimeonline;
		
		// When closing character, we must add the online value to the stats for it to be saved
		addPlayerStats(Ranking.STAT_TOP_ONLINE, currenttimeonline);
		
		final double longestOnlineTime = Math.max(Math.round((currenttimeonline) / 60), getStats().getPlayerStats(Ranking.STAT_TOP_LONGEST_ONLINE_TIME.getRankingInfo()));
		
		// The same for the longest online time, we must calculate it on exit
		addPlayerStats(Ranking.STAT_TOP_LONGEST_ONLINE_TIME, (long) longestOnlineTime);
		
		// We create a dynamic query to update the stats based on the ranking values that use the character_stats table
		try (Connection con = L2DatabaseFactory.getInstance().getConnection();
			PreparedStatement statement1 = con.prepareStatement("REPLACE INTO character_stats VALUES (?, ?, ?)"))
		{
			// Dynamically aggregate each ranking perteneciente to this table with its corresponding value, to achieve a totally dynamic update
			for (Ranking top : Ranking.values())
			{
				if (top.getDbLocation().equalsIgnoreCase("character_stats"))
				{
					// We only save it if the quantity is greater than 0, but we create cells in the db al pedo, the default is 0
	...
	public boolean checkUseMagicConditions(L2Skill skill, boolean forceUse, boolean dontMove, boolean simultaneous)
	{
		...
		if (skill.isOffensive())
		{
			...
			if (target.isDoor())
			{
				...
				if (!target.canBeAttacked() && !target.isAutoAttackable(this) && !getAccessLevel().allowPeaceAttack())
				{
	...
	public final void stopAllEffectsNotStayOnSubclassChange()
	{
		...
		{
			if (!effect.getSkill().isStayOnSubclassChange())
			{
	...
	@Override
	public final void stopAllEffectsNotAvailableInOly()
	{
		super.stopAllEffectsNotAvailableInOly();
		updateAndBroadcastStatus(true);
	}
	...
	public void leaveObserverMode()
	{
		...
		unsetLastLocation();
		sendPacket(new ObservationReturn(getLocation2()));
	...
	public boolean addSubClass(int classId, int classIndex)
	{
		...
		try
		{
			...
			// Restore certification skills if there any
			CertFunc.onCertificationRestore(this);
	...
	public boolean setActiveClass(int classIndex)
	{
		...
		try
		{
			...
			CancelBuffReturnManager.clear(this.getPcData());
	...
	public void addExpAndSp(long addToExp, int addToSp, boolean useVitality)
	{
		if (!getVarB("noExp"))
		{
			...
			// Add the exp acquired to the stats
			if (useVitality)
			{
				if (addToExp > 0)
				{
	...
	private synchronized void cleanup()
	{
		...
		try
		{
			if (!isOnline() && !(this instanceof Autobot))
			{
	...
	{
		return _isHourglassEffected.get();
	}
	
	public void setHourlassEffected(boolean val)
	{
		_isHourglassEffected.set(val);
	}
	...
	{
		return _isRecomTimerActive.get();
	}
	
	public void setRecomTimerActive(boolean val)
	{
		if (_isRecomTimerActive.getAndSet(val) == val)
		{
			return;
		}
	...
	// Actor data that will be used from feature
	// not to mess with pc instance every time
	// many functions will be moved there
	private final PcData _pcdata = PcData.Create(this);
	...
	private PayerData _payerData;
	
	public PayerData getBuyerData()
	{
		if (_payerData == null)
		{
			_payerData = new PayerData(this);
		}
		return _payerData;
	}
	...
	{
		return true;// getQuickVarL(QuickVarType.AUTO_FARMER.getCommand(), 0) > System.currentTimeMillis();
	}

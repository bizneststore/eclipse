...
public class CraftLeaderboard
{
	...
	protected int TASK_DELAY = LeaderboardsConfigs.RANK_CRAFT_INTERVAL;
	...
	public void formRank()
	{
		...
		Broadcast.toAllOnlinePlayers("Attention Players: " + arTop.name + " is the winner for this time with " + arTop.sucess + "/" + arTop.fail + ". Next calculation in " + LeaderboardsConfigs.RANK_CRAFT_INTERVAL + " min(s).");
		if ((winner != null) && (LeaderboardsConfigs.RANK_CRAFT_REWARD_ID > 0) && (LeaderboardsConfigs.RANK_CRAFT_REWARD_COUNT > 0))
		{
			winner.getInventory().addItem("CraftManager", LeaderboardsConfigs.RANK_CRAFT_REWARD_ID, LeaderboardsConfigs.RANK_CRAFT_REWARD_COUNT, winner, null);
			if (LeaderboardsConfigs.RANK_CRAFT_REWARD_COUNT > 1)
			{
				winner.sendPacket(SystemMessage.getSystemMessage(SystemMessageId.EARNED_S2_S1_S).addItemName(LeaderboardsConfigs.RANK_CRAFT_REWARD_ID).addInt(LeaderboardsConfigs.RANK_CRAFT_REWARD_COUNT));
			}
			else
			{
				winner.sendPacket(SystemMessage.getSystemMessage(SystemMessageId.EARNED_ITEM_S1).addItemName(LeaderboardsConfigs.RANK_CRAFT_REWARD_ID));
			}
	...
	public String showHtm(int owner)
	{
		...
		if ((LeaderboardsConfigs.RANK_CRAFT_REWARD_ID > 0) && (LeaderboardsConfigs.RANK_CRAFT_REWARD_COUNT > 0))
		{
			...
			pt += "<font color=\"aadd77\">" + LeaderboardsConfigs.RANK_CRAFT_REWARD_COUNT + " &#" + LeaderboardsConfigs.RANK_CRAFT_REWARD_ID + ";</font>";
		}

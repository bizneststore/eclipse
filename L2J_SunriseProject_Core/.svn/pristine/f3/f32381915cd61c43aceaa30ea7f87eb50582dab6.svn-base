...
public class L2CharacterAI extends AbstractAI
{
	...
	protected boolean tryZigZag(L2Character actor, L2Character mostHate)
	{
		...
		return GeodataUtils.moveWithOffset(actor, mostHate);
	}
				...
				if (npc.getDodge() > 0)
				{
					if (Rnd.get(100) <= npc.getDodge())
					{
						...
						if (Math.sqrt(distance2) <= (60 + combinedCollision))
						{
							...
							Location loc = GeoData.getInstance().moveCheck(npc, newX, newY, posZ);
							moveTo(loc);
							return true;
						}
					}
	...
	protected void onIntentionCast(L2Skill skill, L2Object target)
	{
		...
		{
			_scheduledSkillCast = ThreadPoolManager.getInstance().scheduleGeneral(() -> changeIntentionToCast(skill, target), bowAttackDelay > 0 ? (int) (bowAttackDelay * 0.50) : normalAttackDelay);
			return;
		}
	 ...
	 */
	@Override
	protected void onIntentionMoveTo(Location loc, boolean keyboard)
	{
		if (getIntention() == AI_INTENTION_REST)
		{
			// Cancel action client side by sending Server->Client packet ActionFailed to the L2PcInstance actor
	...
	protected boolean maybeMoveToPawn(L2Object target, int offset)
	{
		...
		if (needToMove)
		{
			...
			if (getFollowTarget() != null)
			{
				...
				if (!_actor.isInsideRadius(target, offset + 100, false, false))
				{
					if ((target instanceof L2Character) && !(target instanceof L2DoorInstance))
					{
						...
						moveToPawn(target, Math.max(offset, 10));
					}
					return true;
				}
				stopFollow();
			...
			if ((target instanceof L2Character) && !(target instanceof L2DoorInstance))
			{
				...
				moveToPawn(target, Math.max(offset - 20, 0));
			}
